...
## 1.4.5: Hashing
Hashing is the process of turning data to fixed-length output no matter how big the data is it’s like encryption but just in one direction there is no way to reverse it , the main use of hashing is to check the integrity of the data so no matter how big is the data it is if 1 bit in the data have changed the output of the hash function wont be the same 

## 1.4.6: Salting
Salting is adding a random piece of data before hashing it makes every hash unique even if tow users have the same password

## 1.4.7: Digital signatures
Is a technic where we combain to cryptographic technics hashing and asymmetric encryption.  
let’s say we want to  transfer a file , sender to recever.  
1 sender hashes the file  
2 sender encrypts the hash using their private key and the result is the digital signature  
3 receiver decrypts the signature using the public key  
4 if the hashes of the file and signature are the same means the message is authentic and unchanged.

## 1.4.8: Key stretching
It Is a technique that strengthens the weak passwords by using multiple couches of hash ( hashing them thousands or millions of times ) so cracking it with brute force attack will take much much more time  
hash ( hash (….(hash ( weak password ) ) ) )

## 1.4.9: Blockchain
Is like a shared non changeable digital notebook where u can add new transactions but not change the old ones, transactions are recorded in blocks and each block is linked to the previous one using hashes , it provide trust without a monitoring authority 

## 1.4.10: Open public ledger
It is the general idea and the concept used blockchain technology , it is a record of transactions that is public, transparent, and accessible to anyone.

## 1.4.11: Certificates
An electronic document used to very the identity of a person or a device ….  
it is issued by a certificate authority (CA)

### 1.4.11.1: Certificate authorities (CAs)
Certificates authorities are trusted organizations that issues digital certificates to websites, businesses and individuals

### 1.4.11.2: Certificate Revocation Lists (CRLs)
is a list of the revoked certificates before the expiration date , a client must download the list and check whether a certificate is still trusted.  
there are multiple reasons for revocation like : key compromise, change in ownership , etc.

### 1.4.11.3: Online Certificate Status Protocol (OCSP)
It is a method to check whether a certificate is revoked. Instead of downloading the CRL the client sends a request to the OCSP server of the authority ( if existed because not every certificate authority have one ) to get the real time state of the certificate is it revoked, valid or unknown .

### 1.4.11.4: Self-signed
Is a certificate that is signed by the own creator instead of trusted CA and because there is  no external validation it is considered unsecure.

### 1.4.11.5: Third-party
It’s the opposite of self signed certificates it is provided by a trusted external CA 

### 1.4.11.6: Root certificate
root certificate is a special digital certificate issued and digitally signed by a Certificate Authority (CA) such as SSL.com. It represents the top level of trust in a certificate hierarchy. Root certificates are sometimes called trust anchors because they are the ultimate source of verification for issued certificates.

### 1.4.11.7: Certificate Signing Request (CSR) Generation
A CSR  is a file generated by a server or user that includes its information like  : public key, domain name …  
It is sent to the CA when requesting a certificate and the CA signes it to create the certificate

### 1.4.11.8: Wildcard
Is a TLS/SSL certificate that secures a domain and it’s subdomains using a single certificates so instead of using certificates for each subdomain of WebsiteExemple.com we secure * WebsiteExemple.com which secure every possible subdomain :  
mail.WebsiteExemple.com  
shop.WebsiteExemple.com  
api.WebsiteExemple.com …
*Sources:*  
